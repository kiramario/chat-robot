#!/usr/bin/env python
# -*- coding: utf-8 -*-
# chat-robot.volcengineTTS
# @Calendar: 2025-04-14 23:28
# @Time: 23:28
...
def test_stream(audio_path: str):
    print("测试流式")
    result = execute_one(
        {
            'id': 1,
            "path": audio_path # "文件路径，尽量用16k采样的wav来模拟流式识别"
        },
        format = "wav"
    )
    print(result)

def run(app_id, access_token, secret_key):
    global App_Id, Access_Token, Secret_Key
    App_Id = app_id
    Access_Token = access_token
    Secret_Key = secret_key
    audio_path = "C:\\Users\\mammon\\Desktop\\sdfsdf\\a_new.wav"
    test_stream(audio_path)


if __name__ == "__main__":
    start = datetime.datetime.now()
    App_Id = "1551906502"
    Access_Token = "IHoqZxeI8O5FGRdM_PAqwpg_EVe7BM1G"
    Secret_Key = "vN7E7Pjummz63r0Auq5NtQL2afY86qI_"

    run(App_Id, Access_Token, Secret_Key)
